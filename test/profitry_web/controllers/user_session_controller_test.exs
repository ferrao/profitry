defmodule ProfitryWeb.UserSessionControllerTest do
  ...
  defp create_login_token(user) do
    {email_token, token} = Profitry.Accounts.UserToken.build_email_token(user, "magic_link")
  ...
  describe "POST /login (sending magic link)" do
    test "sends a link", %{conn: conn, user: user} do
      ...
      assert Repo.get_by!(Profitry.Accounts.UserToken, user_id: user.id).context ==
               "magic_link"
    end
  end
